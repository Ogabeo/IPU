...
from .models import User, UserDegree, UserJob, UserResetPassword, UserScienceTip
...
class UserRegisterForm(forms.ModelForm):
    ...
    #     widget=forms.Select(attrs={'placeholder': 'Kasbingizni tanlang:'}),
    #     required=True,
    #     empty_label="Kasbingizni tanlang"
    # )
    science_tip = forms.ModelChoiceField(
        queryset=UserScienceTip.objects.all(),
        widget=forms.Select(attrs={'placeholder': 'Foydalanuvchi bilim sohasini tanlang:'}),
        required=True,
        empty_label="Foydalanuvchi bilim sohasini tanlang"
    )
    degree = forms.ModelChoiceField(
        queryset=UserDegree.objects.all(),
        widget=forms.Select(attrs={'placeholder': 'Foydalanuvchi darajasini tanlang:'}),
        required=True,
        empty_label="Foydalanuvchi darajasini tanlang"
    )
    confirm_password = forms.CharField(widget=TextInput(attrs={'placeholder':'confirm password...'}), required=True)

    class Meta:
        model = User
        fields = ('username', 'email', 'first_name', 'last_name', 'phone', 'science_tip', 'degree')
